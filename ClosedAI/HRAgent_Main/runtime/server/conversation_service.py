...
from runtime.server.config import Config, WebhookSpec
from runtime.server.conversation_lease import (
    DEFAULT_LEASE_TTL_SECONDS,
    ConversationLeaseHeldError,
)
from runtime.server.event_service import (
...
from runtime.telemetry.sanitizer import model_family, safe_token
from runtime.server.utils import safe_rmtree, utc_now
from context import AgentContext
from core.execution.event import Event
from models.llm import LLM, Message
from core.agent import ACPAgent
from core.agent.acp_file_credentials import CODEX_AUTH_SECRET_NAME
...
from core.conversation.title_utils import (
    extract_message_text,
    generate_title_from_message,
)
from core.agent.credential import CredentialBindingError, VersionedCredentialBinding
from core.execution.event import MessageEvent
...
from mcp_integration.utils import MCPToolProvider
from tools import BROWSER_TOOL_NAME, Tool, is_tool_usable
...
def _create_conversation_worktree(
    ...
    branch = f"HRAgent/{conversation_id}"
...
def _resolve_agent_from_profile(
    ...
    """
    from runtime.persistence.store import (
        get_agent_profile_store,
        get_llm_profile_store,
    )
    from configuration.profiles.resolver import ProfileNotFound, resolve_agent_profile
    from configuration.settings.model import HRAgentsAgentSettings
    ...
    # HRAgents profiles get the discovered catalog minus their ``disabled_skills``
    ...
    if isinstance(settings_config, HRAgentsAgentSettings):
...
class ConversationService:
    ...
    async def _start_conversation(
        ...
        # Capture MCP servers the client explicitly requested before we merge
        # settings-store MCP config. Only these (plus explicitly-attached plugin
        # MCP servers) are eagerly connected on the first turn.
        client_eager_mcp_server_names: list[str] = []
        if request.agent and request.agent.mcp_config:
            client_eager_mcp_server_names = sorted(request.agent.mcp_config.keys())
        ...
        if request.agent_profile_id is not None:
            ...
            # For inline custom agents, we must still load the marketplace mcp_config
            # and merge it so they can use marketplace MCP tools.
            from runtime.persistence import (
                PersistedSettings,
                get_settings_store,
            )
            ...
            if mcp_config:
                ...
                if agent.mcp_config is None:
                    ...
                    merged = {**mcp_config, **agent.mcp_config}
                    agent = agent.model_copy(update={"mcp_config": merged})
                request = request.model_copy(update={"agent": agent})

        request = request.model_copy(
            update={"eager_mcp_server_names": client_eager_mcp_server_names}
        )
        ...
        if initial_message:
            ...
            # Honor SendMessageRequest.run so clients can create a conversation
            # without auto-running and start it explicitly via POST
            # /conversations/{id}/run (mirrors event_router's send_message call).
    ...
    def get_instance(cls, config: Config) -> "ConversationService":
        ...
        from runtime.server.mcp_oauth_store import (
        ...
        from runtime.server.env_bootstrap import load_closedai_env, seed_mcp_secrets

        load_closedai_env()
        seed_mcp_secrets(secrets_store)
    ...
    async def _start_event_service(
        ...
        try:
            ...
            webhook_specs = [*self.webhook_specs, *stored.webhooks]
