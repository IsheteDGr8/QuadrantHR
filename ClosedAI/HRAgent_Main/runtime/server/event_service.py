...
from core.orchestration.goal.prompts import RESUME_PROMPT
from core.conversation.impl.local_conversation import (
...
from core.agent.credential import (
...
from models.llm.streaming import LLMStreamChunk
from mcp_integration.utils import MCPToolProvider
from security.policies.analyzer import SecurityAnalyzerBase
from security.policies.confirmation_policy import ConfirmationPolicyBase, ConfirmRisky, NeverConfirm
from security.policies.llm_analyzer import LLMSecurityAnalyzer
from utilities.async_utils import AsyncCallbackWrapper
from utilities.cipher import Cipher
from utilities.files import atomic_write_text
...
class EventService:
    ...
    def _event_matches_body(self, event: Event, body: str) -> bool:
        ...
        from models.llm.message import content_to_str
    ...
    async def start(self):
        ...
        # Hook execution semantics: HRAgents runs hooks sequentially with early-exit
        ...
        conversation = LocalConversation(
            ...
            eager_mcp_server_names=self.stored.eager_mcp_server_names,
        )

        conversation.set_confirmation_policy(self.stored.confirmation_policy)
        conversation.set_security_analyzer(self.stored.security_analyzer)
        self._apply_hr_guardrail_defaults(conversation)
    ...
    def _apply_hr_guardrail_defaults(self, conversation) -> None:
        """Restore HITL on resume: old chats stored NeverConfirm / no analyzer."""
        ...
        if isinstance(policy, NeverConfirm):
            ...
            self.stored.confirmation_policy = policy
        if conversation.state.security_analyzer is None:
            analyzer = LLMSecurityAnalyzer()
            conversation.set_security_analyzer(analyzer)
            self.stored.security_analyzer = analyzer
        tags = getattr(conversation.state, "tags", None) or {}
        if str(tags.get("readonly", "")).lower() in ("1", "true", "yes"):
            conversation._state.agent_state = {
                **conversation._state.agent_state,
                "hr_read_only": True,
            }
