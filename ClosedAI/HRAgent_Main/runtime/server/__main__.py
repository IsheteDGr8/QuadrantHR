...
from runtime.server.logging_config import LOGGING_CONFIG
...
def _auth_enabled() -> bool:
    ...
    """
    from runtime.server.config import load_config
...
def check_browser():
    ...
    The browser toolset was removed during the cleanup, so browser
    functionality is unavailable and the check always fails.
    """
    print("Browser check failed: the browser toolset is not available in this build.")
    return False
...
def main() -> None:
    ...
    from runtime.server.env_bootstrap import load_closedai_env

    load_closedai_env()

    parser = argparse.ArgumentParser(description="HRAgents Agent Server App")
    parser.add_argument(
        "--host",
        default=None,
        help=(
    ...
    print(f"Starting HRAgents Agent Server on {host}:{args.port}")
