...
from models.llm.utils.model_info import get_litellm_model_info
from configuration.settings.metadata import SettingProminence, field_meta
from utilities.pydantic_secrets import serialize_secret, validate_secret


if TYPE_CHECKING:  # type hints only, avoid runtime import cycle
    from models.llm.auth import SupportedVendor
    from models.llm.auth.openai import OpenAIAuthMethod
    from tools.tool import ToolDefinition

from models.llm.auth.openai import transform_for_subscription
...
from models.llm.mixins.non_native_fc import NonNativeToolCallingMixin
from models.llm.options.chat_options import select_chat_options
from models.llm.options.responses_options import select_responses_options
from models.llm.streaming import (
    AnyTokenCallbackType,
    TokenCallbackType,
    _invoke_token_callback,
)
from models.llm.utils.image_inline import (
    amaybe_inline_image_urls,
    maybe_inline_image_urls,
)
from models.llm.utils.image_resize import maybe_resize_messages_for_provider
from models.llm.utils.litellm_provider import infer_litellm_provider
from models.llm.utils.metrics import Metrics
from models.llm.utils.model_features import get_features
from models.llm.utils.HRAgent_provider import (
    LiteLLMCallKwargs,
    canonicalize_HRAgent_llm_payload,
    litellm_call_kwargs,
)
from models.llm.utils.retry_mixin import RetryMixin
from models.llm.utils.telemetry import Telemetry
from models.llm.utils.vertex_preflight import assert_vertex_sdk_available
from runtime.telemetry.logger import ENV_LOG_DIR, get_logger
...
# Minimum context window size required for HRAgents to function properly.
...
class LLM(BaseModel, RetryMixin, NonNativeToolCallingMixin):
    """Language model interface for HRAgents agents.
    ...
    model_canonical_name: str | None = Field(
        ...
        description=(
            ...
            "The HRAgents SDK maintains a model feature registry that "
    ...
    @classmethod
    def _message_has_user_visible_text(cls, item: Any) -> bool:
        if cls._responses_item_type(item) != "message":
            return False
        content = (
        ...
        for part in content or []:
            ...
            if part_type == "output_text" and text and str(text).strip():
                return True
        return False

    @classmethod
    def _filter_user_visible_messages(cls, output: list[Any] | None) -> list[Any]:
        """Completed assistant message items with user-visible text only."""
        return [item for item in (output or []) if cls._message_has_user_visible_text(item)]

    @classmethod
    def _count_tool_calls(cls, output: list[Any] | None) -> int:
        return sum(
            1 for item in (output or []) if cls._responses_item_type(item) == "function_call"
        )

    @staticmethod
    def _read_incomplete_reason(resp: ResponsesAPIResponse) -> str | None:
    ...
    @staticmethod
    def _coerce_responses_api_response(resp: Any) -> ResponsesAPIResponse:
        if isinstance(resp, ResponsesAPIResponse):
            return resp
        if isinstance(resp, dict):
            return ResponsesAPIResponse.model_validate(resp)
        raise TypeError(
            f"Expected ResponsesAPIResponse for stream terminal event, got {type(resp)!r}"
        )
    ...
    def _merge_stream_output(
        ...
        merged: list[Any] = list(resp.output or [])
        if not merged and collected_output_items:
            merged = list(collected_output_items)
        return merged

    def _log_incomplete_diagnostic(
        self,
        resp: ResponsesAPIResponse,
        *,
        tools_used: int,
        ...
        reason = self._read_incomplete_reason(resp)
        logger.warning(
            "Responses stream ended incomplete%s: reason=%s model=%s "
            "max_output_tokens=%s tools_in_output=%d status=%s "
            "incomplete_details=%r provider_error=%r",
            " (returning degraded user-visible text)" if degraded else "",
            reason or "unknown",
            self.model,
    ...
    def _raise_incomplete_response_error(
        self,
        resp: ResponsesAPIResponse,
        merged_output: list[Any],
    ) -> None:
        reason = self._read_incomplete_reason(resp)
        tools_used = self._count_tool_calls(merged_output)
        self._log_incomplete_diagnostic(resp, tools_used=tools_used, degraded=False)
    ...
    def _finalize_stream_response(
        ...
        Raises:
            LLMNoResponseError: Missing terminal event or hard provider failure.
            LLMIncompleteResponseError: Incomplete stream with no user-visible text.
        """
        ...
        if isinstance(completed_response, ResponseIncompleteEvent):
            incomplete_resp = self._coerce_responses_api_response(
                completed_response.response
            )
            merged_output = self._merge_stream_output(
                incomplete_resp, collected_output_items
            )
            degraded_messages = self._filter_user_visible_messages(merged_output)
            if degraded_messages:
                self._log_incomplete_diagnostic(
                    incomplete_resp,
                    tools_used=self._count_tool_calls(merged_output),
                    degraded=True,
                )
                incomplete_resp.output = degraded_messages
                assert self._telemetry is not None
                self._telemetry.on_response(incomplete_resp)
                return incomplete_resp
            self._raise_incomplete_response_error(incomplete_resp, merged_output)

        if isinstance(completed_response, ResponseFailedEvent):
            failed_resp = self._coerce_responses_api_response(
                completed_response.response
            )
            provider_error = getattr(failed_resp, "error", None)
            logger.error(
                "Responses stream failed: model=%s status=%s provider_error=%r",
                self.model,
                getattr(failed_resp, "status", None),
                provider_error,
            )
            raise LLMNoResponseError("The model provider reported a failed response.")

        if not isinstance(completed_response, ResponseCompletedEvent):
            provider_error = getattr(
                getattr(completed_response, "response", None), "error", None
            )
            raise LLMNoResponseError(
                f"Unexpected completed event: {type(completed_response)}"
                f"{f' -- provider error: {provider_error}' if provider_error else ''}"
            )

        completed_resp = self._coerce_responses_api_response(completed_response.response)
    ...
    def completion(
        ...
        **kwargs,
        ...
        Example:
            ```python
            from models.llm import Message, TextContent
    ...
    def _get_litellm_auth_values(self) -> tuple[str | None, dict[str, str]]:
        ...
        if self.is_subscription:
            from models.llm.auth.openai import OpenAISubscriptionAuth
    ...
    def _validate_context_window_size(self) -> None:
        """Validate that the context window is large enough for HRAgents."""
    ...
    def from_persisted(cls, data: Any, *, context: dict[str, Any] | None = None) -> LLM:
        ...
        payload = canonicalize_HRAgent_llm_payload(payload)
        llm = cls.model_validate(payload, context=context)
        if llm.auth_type == "subscription":
            from models.llm.auth.openai import (
    ...
    def subscription_login(
        ...
        **llm_kwargs,
        ...
        Example:
            ...
            ```
        """
        from models.llm.auth.openai import subscription_login
