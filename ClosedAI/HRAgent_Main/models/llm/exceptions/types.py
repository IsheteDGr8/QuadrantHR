...
class LLMIncompleteResponseError(LLMError):
    """Responses API stream ended with ``response.incomplete`` and no usable text."""
    ...
    def __init__(
        ...
        *,
        ...
        if message is None:
            message = self._user_message(reason)
        super().__init__(message)

    @staticmethod
    def _user_message(reason: str | None) -> str:
        if reason == "max_output_tokens":
            return (
                "The model hit the output token limit before finishing its reply. "
                'Send "continue" or ask for a shorter summary.'
            )
        if reason == "content_filter":
            return "The model response was blocked by a content filter."
        return "The model response was incomplete before it could finish."
...
class LLMContextWindowTooSmallError(LLMError):
    """Raised when the model's context window is too small for HRAgents to work."""
