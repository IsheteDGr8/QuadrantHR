"""Discovery of locally-available plugins for HRAgents SDK.
...
from plugins.installed import (
    get_installed_plugins_dir,
    load_installed_plugins,
)
from skills.skill import _find_git_repo_root
...
def _load_plugins_from_dir(plugins_dir: Path, skip_dirs: set[Path]) -> list['Plugin']:
    ...
    ``~/.HRAgent/plugins/installed`` — a child of ``~/.HRAgent/plugins`` — and
...
def load_user_plugins() -> list[Plugin]:
    ...
    Scans ``~/.agents/plugins`` then ``~/.HRAgent/plugins`` (earlier wins on a
    name conflict), then appends enabled installed plugins from
    ``~/.HRAgent/plugins/installed`` (lower precedence than directory plugins).
...
def load_project_plugins(work_dir: str | Path) -> list[Plugin]:
    ...
    Scans ``{root}/.agents/plugins`` and ``{root}/.HRAgent/plugins`` for the
