...
from mcp_integration.config import MCPServer, coerce_mcp_config
...
class Plugin(BaseModel):
    ...
    hooks: 'HookConfig | None' = Field(
        default=None, description="Event handlers configured in hooks.json"
    )
    ...
    def add_mcp_config_to(
        ...
        # Backfill each server's `description` from the owning plugin's
        # manifest description when the server itself doesn't declare one
        # (individual .mcp.json entries typically don't -- only the plugin
        # manifest does). This is the only per-server description available
        # to a lazily-activatable MCP catalog (see ActivateIntegrationTool),
        # which needs *some* human-readable hint to route on, not just a
        # bare server name.
        if self.manifest.description:
            plugin_servers = {
                name: (
                    server.model_copy(update={"description": self.manifest.description})
                    if server.description is None
                    else server
                )
