...
from core.agent.acp_agent import ACPAgent
from core.agent.base import AgentBase
from context.condenser import CondenserBase, LLMSummarizingCondenser
...
from core.conversation.secret_registry import SecretValue
from core.conversation.state import (
    ConversationExecutionStatus,
    ConversationState,
)
from core.conversation.stuck_detector import StuckDetector
from core.conversation.title_utils import generate_conversation_title
...
from core.agent.credential import CredentialBindingError
...
from core.execution.hooks import HookConfig, HookEventProcessor, create_hook_callback
from utilities.io import FileStore, LocalFileStore
from models.llm import LLM, Message, TextContent, content_to_str
from models.llm.auth.openai import create_subscription_llm_from_config
from models.llm.llm import LLMCallContext
from models.llm.llm_profile_store import LLMProfileStore
from models.llm.llm_registry import LLMRegistry
...
from mcp_integration.exceptions import MCPError, MCPReauthenticationRequiredError
from mcp_integration.utils import (
    DefaultMCPToolProvider,
    MCPToolProvider,
    ToolsChangedCallback,
)
from runtime.telemetry.observability.laminar import observe
from plugins import (
    Plugin,
    PluginSource,
...
from security.policies.analyzer import SecurityAnalyzerBase
...
from tools.builtins import InvokeSkillTool
from tools.client_tool import ClientToolSpec
from tools.schema import Action, Observation
...
class LocalConversation(BaseConversation):
    ...
    def __init__(
        ...
        eager_mcp_server_names: list[str] | None = None,
        **_: object,
    ):
        ...
        # Non-persisted (process-local) cache of which tool names came from
        # which activated MCP server -- see activate_mcp_server. Persisted
        # state only needs *which servers* were activated
        # (state.activated_mcp_servers); this in-memory index just avoids
        # re-deriving "already active" tool names within the same process.
        self._activated_mcp_server_tool_names: dict[str, list[str]] = {}
        # Populated by _ensure_plugins_loaded(): names of MCP servers added
        # by ambient (installed-plugin) merging rather than explicit
        # request. Empty until plugins are loaded.
        self._lazy_mcp_server_names: set[str] = set()
        # MCP servers to connect eagerly on first turn (client-requested only;
        # settings-merged servers are lazy). Populated from StoredConversation.
        self._eager_mcp_server_names: set[str] = set(eager_mcp_server_names or [])
        ...
        if resolved_client_tools:
            from tools.client_tool import register_client_tools
    ...
    def _recover_persisted_client_tools(
        ...
        from core.conversation.persistence_const import BASE_STATE
        from tools.client_tool import extract_client_tool_specs
    ...
    def _ensure_plugins_loaded(self) -> None:
        ...
        mcp_before_explicit_plugins = set(merged_mcp.keys())
        ...
        # Eager-connect only client-requested MCP servers (captured before
        # settings-store merge at conversation create) plus any MCP added by
        # explicitly-attached plugins above. Settings-merged servers already
        # present in agent.mcp_config and ambient installed plugins stay lazy
        # (ActivateIntegrationTool activates on demand).
        plugin_added_mcp_names = set(merged_mcp.keys()) - mcp_before_explicit_plugins
        explicit_mcp_names = set(self._eager_mcp_server_names) | plugin_added_mcp_names
        ...
        # Everything added by ambient (installed/local) plugins, not present
        # in the explicit set captured above -- these are lazily activated
        # via ActivateIntegrationTool rather than eagerly connected below.
        self._lazy_mcp_server_names = set(merged_mcp.keys()) - explicit_mcp_names
    ...
    def _runtime_mcp_tools(
        ...
        *,
        ...
        if not mcp_config:
            return []
        try:
            from runtime.server.env_bootstrap import overlay_process_env_on_mcp

            mcp_config = overlay_process_env_on_mcp(mcp_config)
            client = self._mcp_tool_provider.create_tools(
                mcp_config,
                _RUNTIME_MCP_TIMEOUT_SECS,
                on_tools_changed=on_tools_changed,
            )
        except Exception as exc:  # noqa: BLE001 - never brick the conversation
    ...
    def _runtime_mcp_tools_for_agent(self) -> list[ToolDefinition]:
        """Eagerly connect only the *explicit* MCP servers for this conversation.

        Ambient (installed-plugin) servers are deliberately excluded here --
        see ``_lazy_mcp_server_names`` and ``activate_mcp_server``. They stay
        configured (still in ``agent.mcp_config``, still get correct
        credentials, still show up in the ``activate_integration`` catalog)
        but are not connected-to/fetched until a conversation actually asks
        for them.
        """
        if not self.agent.supports_HRAgent_tools or not self.agent.mcp_config:
            return []
        eager_config = {
            name: server
            for name, server in self.agent.mcp_config.items()
            if name not in self._lazy_mcp_server_names
        }
        if not eager_config:
            return []
        tools = self._runtime_mcp_tools(
            eager_config,
            on_tools_changed=self.agent._on_mcp_tools_changed,
        )
        # Best-effort per-server bookkeeping so a redundant
        # `activate_integration("hr")` call resolves to real tool names
        # instead of an empty (but harmless) list. Names aren't tagged with
        # their owning server on the ToolDefinition itself, so this can only
        # attribute correctly when exactly one eager server is configured
        # (overwhelmingly the common case -- one small local server); with
        # more than one, tools are still fully registered and usable, this
        # bookkeeping is just informational for activate_mcp_server's
        # already-active response.
        if len(eager_config) == 1:
            (only_name,) = eager_config.keys()
            self._activated_mcp_server_tool_names[only_name] = [t.name for t in tools]
        return tools

    def activate_mcp_server(self, name: str) -> tuple[list[str], str | None]:
        """Connect one configured-but-not-yet-active MCP server and register its tools.

        Called by ``ActivateIntegrationTool`` (and internally, on resume, to
        restore whatever a conversation had already activated). Installed MCP
        servers are *configured* (present in ``agent.mcp_config``, merged from
        both explicit config and ambient plugins) but not eagerly connected --
        see ``_ensure_agent_ready``, which registers this activation tool
        instead of calling ``_runtime_mcp_tools_for_agent`` for the whole set.
        This keeps a large installed catalog from being fetched into every
        conversation's context regardless of relevance.

        Returns ``(tool_names, error)``: on success, the names of tools just
        registered and ``None``; on failure (unknown name, unreachable
        server, missing/expired OAuth session), an empty list and a clear,
        user-facing error string. Never raises, never opens a browser --
        reuses the same ``RuntimeOnlyOAuth`` path as every other runtime MCP
        connection (see ``mcp_integration.utils._prepare_mcp_config``), so an
        unauthenticated server fails with a structured message pointing at
        MCP Settings rather than a popup.
        """
        server = self.agent.mcp_config.get(name)
        if server is None:
            available = sorted(self.agent.mcp_config.keys())
            return [], (
                f"Unknown integration '{name}'. Installed integrations: "
                f"{', '.join(available) if available else '<none>'}."
            )

        if (
            name in self._activated_mcp_server_tool_names
            or name not in self._lazy_mcp_server_names
        ):
            # Either already lazily activated in this process, or never was
            # lazy in the first place (an explicit/eager server, e.g. the
            # local `hr` data server -- already connected during
            # _ensure_agent_ready, so there's nothing to do here beyond
            # confirming it's ready).
            return self._activated_mcp_server_tool_names.get(name, []), None

        # Gmail MCP is allowed for inbox reads (search_threads / get_thread /
        # list_*). Prefer list_emails client tool when available — it uses the
        # Gmail REST API and avoids MCP connect timeouts. Sending still goes
        # through send_email (create_draft cannot attach files reliably).
        activation_timeout = (
            max(_RUNTIME_MCP_TIMEOUT_SECS, 90) if name == "gmail" else _RUNTIME_MCP_TIMEOUT_SECS
        )

        try:
            from runtime.server.env_bootstrap import overlay_process_env_on_mcp

            server = overlay_process_env_on_mcp({name: server})[name]
            client = self._mcp_tool_provider.create_tools(
                {name: server},
                activation_timeout,
                on_tools_changed=self.agent._on_mcp_tools_changed,
            ...
            return [], (
                f"Integration '{name}' needs re-authentication -- its OAuth "
                "session is missing, expired, or invalid. Reconnect it from "
                "MCP Settings before it can be used."
            )
        except Exception as exc:  # noqa: BLE001 - surface to the agent, don't crash
            logger.warning(
                "Failed to activate MCP server '%s': %s", name, exc, exc_info=True
            )
            return [], (
                f"Integration '{name}' is configured but not reachable right "
                f"now ({exc}). Use /api/mcp/test or MCP Settings to diagnose."
            )

        tools = list(client.tools)
        if not tools:
            return [], f"Integration '{name}' connected but exposes no tools."

        try:
            self.agent.add_runtime_tools(tools)
        except ValueError as exc:
            # Name collision against something already registered (e.g. a
            # client tool, or another server's tool with the same name).
            # The connection succeeded, so don't leave it dangling -- close
            # it and report clearly rather than crashing the tool call.
            self._close_runtime_tools(tools)
            return [], f"Integration '{name}' could not be activated: {exc}"
        tool_names = [t.name for t in tools]
        self._activated_mcp_server_tool_names[name] = tool_names
        if name not in self._state.activated_mcp_servers:
            self._state.activated_mcp_servers.append(name)
        return tool_names, None

    def _restore_activated_mcp_servers(self) -> None:
        """Re-activate MCP servers a resumed conversation had already opted into.

        ``activated_mcp_servers`` persists which servers were actually
        connected (not merely configured) before a restart/resume -- without
        this, a resumed conversation would silently lose access to tools the
        model (and any persisted tool-call history) already depends on,
        while still re-fetching only what was actually in use rather than
        the full installed catalog.
        """
        for name in list(self._state.activated_mcp_servers):
            if name not in self.agent.mcp_config:
                continue  # server was uninstalled/renamed since activation
            try:
                _, error = self.activate_mcp_server(name)
            except Exception:
                # Restoring one server must never abort conversation resume
                # for the rest -- activate_mcp_server already handles the
                # expected MCP/connection error types itself; this is a
                # last-resort net for anything else (e.g. a duplicate-name
                # collision against a tool registered some other way).
                logger.warning(
                    "Unexpected error restoring MCP server '%s' on resume",
                    name,
                    exc_info=True,
                )
                continue
            if error:
                logger.warning(
                    "Could not restore previously-activated MCP server '%s' "
                    "on resume: %s",
                    name,
                    error,
                )
    ...
    def _register_file_based_agents(self) -> None:
        ...
        Registration order (highest to lowest priority):
          ...
          3. Project-level file agents (`{project}/.agents/agents/*.md`,
                then `{project}/.HRAgent/agents/*.md`)
          4. User-level file agents (`~/.agents/agents/*.md`,
                then `~/.HRAgent/agents/*.md`)
        """
    ...
    def _ensure_agent_ready(self) -> None:
        ...
        with self._state:
            ...
            try:
                if self.agent.supports_HRAgent_tools:
                    ...
                    # Resuming a conversation that had already activated some
                    # ambient MCP servers (see activate_mcp_server): restore
                    # just those, not the full installed catalog, so tools
                    # the model (and any persisted tool-call history) already
                    # depends on don't silently disappear across a restart.
                    self._restore_activated_mcp_servers()
    ...
    def switch_acp_model(self, model: str) -> None:
        ...
        Unlike :meth:`switch_llm`, which swaps HRAgents' own LLM object, this
    ...
    def interrupt(self) -> None:
        ...
        :class:`~core.execution.event.InterruptEvent`.
    ...
    def ask_agent(self, question: str) -> str:
        ...
        from core.agent.utils import make_llm_completion, prepare_llm_messages
