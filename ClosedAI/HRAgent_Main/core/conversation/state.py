...
from core.conversation.event_store import ROOT_PARENT_ID, EventLog
from core.conversation.fifo_lock import FIFOLock
from core.conversation.persistence_const import BASE_STATE, EVENTS_DIR
...
from core.execution.event.base import Event
from core.execution.event.types import EventID
from core.execution.hooks import HookConfig
from utilities.io import FileStore, InMemoryFileStore, LocalFileStore
...
from utilities.models import HRAgentsModel
...
class ConversationState(HRAgentsModel):
    ...
    activated_mcp_servers: list[str] = Field(
        default_factory=list,
        description=(
            "Names of MCP servers (from `agent.mcp_config`) explicitly "
            "activated via the `activate_integration` tool -- i.e. actually "
            "connected-to and materialized as callable tools, as opposed to "
            "merely configured/installed. Parallel to `invoked_skills`: "
            "installed MCP servers are not eagerly connected at conversation "
            "start (their full tool catalogs can be large), so this tracks "
            "which ones a given conversation actually opted into, both to "
            "avoid re-activating the same server twice and to restore the "
            "same set of active tools on conversation resume."
        ),
    )
    ...
    agent_state: dict[str, Any] = Field(
        ...
        "See https://docs.HRAgent.dev/sdk/guides/convo-persistence#how-state-persistence-works",
    )
    ...
    def _resolve_active_leaf(self) -> EventID | None:
        ...
        from core.execution.event.conversation_state import (
