...
from core.agent.acp_agent import ACPAgent as ACPAgent
...
from core.execution.hooks import HookConfig
from models.llm.message import ImageContent, Message, TextContent
from plugins import PluginSource
from security.validation.secret import SecretSource
...
class ConversationWebhookSpec(BaseModel):
    """Per-conversation webhook configuration.

    Mirrors the agent-server WebhookSpec shape without importing server runtime
    modules into the SDK request model.
    """

    event_buffer_size: int = Field(default=5, ge=1)
    base_url: str
    headers: dict[str, str] = Field(default_factory=dict)
    flush_delay: float = Field(default=30.0, gt=0)
...
class StartConversationRequest(BaseModel):
    ...
    HRAgents agents and ACP agents. Clients may provide either a concrete
    ...
    webhooks: list[ConversationWebhookSpec] = Field(
        default_factory=list,
        description=(
            "Webhook subscribers scoped to this conversation. These supplement "
            "server-level webhooks and are used by hosts that need lifecycle or "
            "execution events for one conversation without reinitializing the "
            "whole server."
        ),
    )
    ...
    eager_mcp_server_names: list[str] = Field(
        default_factory=list,
        description=(
            "MCP server names to connect eagerly on the first turn. Set by the "
            "server from the client's agent.mcp_config before settings-store MCP "
            "servers are merged in. Settings-merged and ambient-installed servers "
            "stay lazy (activated via ActivateIntegrationTool on demand)."
        ),
    )
    ...
    title_llm_profile: str | None = Field(
        ...
        description=(
            ...
            "is loaded from LLMProfileStore (~/.HRAgent/profiles/) and used "
    ...
    def _populate_agent_from_settings(cls, data: Any) -> Any:
        ...
        if not has_profile_id:
            if payload.get("agent") is None and has_agent_settings:
                from configuration.settings.model import validate_agent_settings
...
class StartACPConversationRequest(StartConversationRequest):
    ...
    HRAgents agents and ACP agents through the same request contract.
    """
