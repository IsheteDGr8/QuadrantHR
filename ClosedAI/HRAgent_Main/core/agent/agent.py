...
import security.policies.analyzer as analyzer
import security.policies.risk as risk
...
from core.agent.critic_mixin import CriticMixin
from core.agent.parallel_executor import ParallelToolExecutor
from core.agent.response_dispatch import (
    LLMResponseType,
...
from context.prompts.presets import PromptPreset, create_registry
...
from core.conversation.state import ConversationExecutionStatus
...
from runtime.telemetry.observability.laminar import (
    maybe_init_laminar,
    observe,
    should_enable_observability,
)
from runtime.telemetry.observability.utils import extract_action_name
from tools import (
    Action,
    Observation,
)
...
from tools.builtins.vision_inspect import VISION_INSPECT_TOOL_NAME
...
class Agent(CriticMixin, ResponseDispatchMixin, AgentBase):
    """Main agent implementation for HRAgents.
    ...
    Attributes:
        ...
        system_prompt: Inline system prompt string. When provided the agent
            ...
            **Not recommended** unless you know what you are doing (e.g.
            ...
            this will override HRAgents' built-in system instructions.
    ...
    def init_state(
        ...
        # See: https://github.com/HRAgents/software-agent-sdk/issues/1785
        #
    ...
    def step(
        ...
        # See https://github.com/HRAgents/software-agent-sdk/issues/3053.
    ...
    def _requires_user_confirmation(
        ...
        Rules:
            ...
            5. `ActivateIntegrationAction`s never require confirmation, alone
               or batched with other actions (see below)
        """
        ...
        # ActivateIntegrationAction (see tools/builtins/activate_integration.py)
        # only registers more tools as callable on the running agent -- unlike
        # every other action type it never reads or writes anything external,
        # so its risk is fixed regardless of what the model self-reports for
        # it. Excluding it here (rather than only exempting a *sole*
        # ActivateIntegrationAction, as FinishAction/ThinkAction are above)
        # matters because the model frequently batches it with the real
        # action it unlocks (e.g. activate slack + search channels in one
        # turn) -- without this, that real action's own risk assessment
        # would still correctly gate the batch, but the activation step
        # itself would otherwise add an extra, redundant confirmation for an
        # action with nothing to confirm.
        risk_relevant_events = [
            e
            for e in action_events
            if not isinstance(e.action, ActivateIntegrationAction)
        ]
        if len(risk_relevant_events) == 0:
            return False

        # Hard rails: use the event's security_risk (forced HIGH for sends/
        # writes) even when the analyzer is missing. NeverConfirm cannot skip
        # outbound tools — see security.policies.hr_guardrails.
        if should_pause_for_confirmation(
            state.confirmation_policy,
            risk_relevant_events,
    ...
    def _extract_security_risk(
        ...
        # Guardrail: irreversible / outbound tools are ALWAYS HIGH, even if the
        # model omits security_risk or falsely labels them LOW. ConfirmRisky
        # then pauses for human approval before execution.
        if is_forced_high_risk_tool(tool_name):
    ...
    @staticmethod
    def _is_forced_high_risk_tool(tool_name: str) -> bool:
        """Tools that must never execute without ConfirmRisky HITL."""
        return is_forced_high_risk_tool(tool_name)
            ...
            tool_call = tool_call.model_copy(
                update={
                    "arguments": json.dumps(
                        {
                            "_HRAgent_malformed_tool_call": True,
                            "error": error,
                        }
                    )
                }
    ...
    def _execute_action_event(
        ...
        if conversation_is_read_only(conversation) and is_mutating_hr_tool(
            action_event.tool_name
        ):
            return [
                AgentErrorEvent(
                    error=READ_ONLY_BLOCK_MESSAGE,
                    tool_name=tool.name,
    ...
    def _log_context_window_exceeded_warning(self) -> None:
        ...
        logger.warning(
            ...
            "https://docs.HRAgent.dev/sdk/guides/context-condenser\n"
            "=" * 80
        )
