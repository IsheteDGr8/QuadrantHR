...
from context.condenser import CondenserBase
from context.prompts.presets import PromptPreset, create_registry
from context.prompts.prompt import render_template
from context.prompts.section import Platform, PromptContext
from core.critic import CriticBase
from models.llm import LLM
from models.llm.utils.model_prompt_spec import get_model_prompt_spec
...
from tools.builtins import ActivateIntegrationTool, InvokeSkillTool
from tools.builtins.vision_inspect import (
    VisionInspectTool,
    has_vision_profile_available,
)
from utilities.models import DiscriminatedUnionMixin


if TYPE_CHECKING:
    from core.conversation import ConversationState, LocalConversation
...
# -- SOUL.md loader -------------------------------------------------------
# SOUL.md is the agent's identity file (~/.HRAgent/SOUL.md).  When present
# it replaces the default identity in the system prompt.

_SOUL_PATH = os.path.join(os.path.expanduser("~"), ".HRAgent", "SOUL.md")
_DEFAULT_SOUL = (
    "You are HRAgents agent, a helpful AI assistant that can interact"
...
def _load_soul_md() -> str:
    """Load ``~/.HRAgent/SOUL.md``, falling back to the built-in default."""
    try:
...
class AgentBase(DiscriminatedUnionMixin, ABC):
    """Abstract base class for HRAgents agents.
    ...
    system_prompt: str | None = Field(
        ...
        description=(
            ...
            "task).  Setting this will override HRAgents' built-in system "
    ...
    def _initialize(
        ...
        # Auto-attach `ActivateIntegrationTool` iff any MCP servers are
        # configured. Their tools are no longer eagerly connected/registered
        # here (see LocalConversation._ensure_agent_ready) -- this tool is
        # the model's only way to request a given one on demand, so it must
        # be present whenever there's anything to activate.
        if self.mcp_config and ActivateIntegrationTool.__name__ not in default_tool_names:
            default_tool_names.append(ActivateIntegrationTool.__name__)
            logger.debug(
                "Auto-attached %s (%d MCP server(s) configured)",
                ActivateIntegrationTool.__name__,
                len(self.mcp_config),
            )
    ...
    @property
    def supports_HRAgent_tools(self) -> bool:
        """``True`` if HRAgents can inject tools into this agent.

        ``False`` for :class:`~core.agent.acp_agent.ACPAgent` — the
        ACP server manages its own toolset.
        """
        return True

    @property
    def supports_HRAgent_mcp(self) -> bool:
        """``True`` if HRAgents can create in-process MCP tools for this agent.

        ``False`` for :class:`~core.agent.acp_agent.ACPAgent` — ACP
    ...
    def supports_condenser(self) -> bool:
        """``True`` if HRAgents context condensing is supported for this agent.
    ...
    @property
    def agent_kind(self) -> Literal["HRAgent", "acp"]:
