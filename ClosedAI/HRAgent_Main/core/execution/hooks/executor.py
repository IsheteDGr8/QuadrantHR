...
from core.conversation.visualizer import ConversationVisualizerBase
from core.execution.hooks.config import HookDefinition, HookType
from core.execution.hooks.types import HookDecision, HookEvent
from runtime.telemetry.observability.laminar import observe
from utilities import sanitized_env
...
class HookExecutor:
    ...
    def _execute_agent_hook(
        ...
        from core.agent import Agent  # type: ignore[attr-defined]
    ...
    def execute(
        ...
        hook_env["HRAgentS_PROJECT_DIR"] = self.working_dir
        hook_env["HRAgentS_SESSION_ID"] = event.session_id or ""
        hook_env["HRAgentS_EVENT_TYPE"] = event.event_type
        if event.tool_name:
            hook_env["HRAgentS_TOOL_NAME"] = event.tool_name
