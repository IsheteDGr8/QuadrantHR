...
from models.llm import TextContent
...
from security.policies import risk
from skills.utils import expand_variable_references
from tools import (
    Action,
    Observation,
    ToolAnnotations,
...
from tools.schema import Schema, _process_schema_node
from tools.tool import DeclaredResources, _prioritize_schema_fields
...
class MCPToolExecutor(ToolExecutor):
    ...
    async def call_tool(self, action: MCPToolAction) -> MCPToolObservation:
        ...
        try:
            ...
            # Guardrail: never auto-send Gmail drafts. Delivery must go through
            # send_email (client tool) + ConfirmRisky HITL. create_draft alone
            # only creates a draft — do not call users.drafts.send here.
            if (
                not result.isError
                and is_gmail_create_draft_tool(self.tool_name, self.client)
            ):
                draft_id: str | None = None
                for block in observation.content:
                    if isinstance(block, TextContent) and block.text:
                        draft_id = extract_draft_id(block.text)
                        if draft_id:
                            break
                if draft_id:
                    observation = MCPToolObservation.from_text(
                        text=(
                            f"Gmail draft created (draft id: {draft_id}). "
                            "It was NOT sent. To deliver mail, call send_email "
                            "with security_risk=HIGH and wait for human approval."
                        ),
                        is_error=False,
                        tool_name=self.tool_name,
                    )
                    ...
                    observation = MCPToolObservation.from_text(
                        text=(
                            "Gmail create_draft completed but no draft id was "
                            "found. Do NOT claim the email was sent. Prefer "
                            "send_email with security_risk=HIGH for delivery."
                        ),
                        is_error=False,
                        tool_name=self.tool_name,
                    )
...
class MCPToolDefinition(ToolDefinition[MCPToolAction, MCPToolObservation]):
    ...
    def declared_resources(self, action: Action) -> DeclaredResources:
        """MCP tools do not contend for local single-threaded resources."""
    ...
    def action_from_arguments(self, arguments: dict[str, Any]) -> MCPToolAction:
        ...
        # internal to HRAgents and not part of the MCP tool schema
    ...
    def _get_tool_schema(
        ...
        HRAgents-specific fields (``security_risk``, ``summary``) directly.

        See: https://github.com/HRAgents/software-agent-sdk/issues/3955
        """
