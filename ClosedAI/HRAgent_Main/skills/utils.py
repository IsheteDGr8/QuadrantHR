...
if TYPE_CHECKING:
    from skills.skill import Skill, SkillResources
...
def load_and_categorize(
    ...
    # AgentSkills (SKILL.md directories) are a separate category from HRAgents skills.
...
def get_skills_cache_dir() -> Path:
    ...
    Returns:
        Path to the skills cache directory (~/.HRAgent/cache/skills).
    """
    cache_dir = Path.home() / ".HRAgent" / "cache" / "skills"
...
def update_skills_repository(
    ...
    Uses the shared git caching infrastructure from utilities.git.
...
def discover_skill_resources(skill_dir: Path) -> SkillResources:
    ...
    """
    # Import here to avoid circular dependency
    from skills.skill import SkillResources
