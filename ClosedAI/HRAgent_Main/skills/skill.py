...
from skills.exceptions import SkillError, SkillValidationError
from skills.execute import render_content_with_commands
from skills.trigger import (
    KeywordTrigger,
    PathTrigger,
    TaskTrigger,
)
from skills.types import InputMetadata
...
from utilities import DEFAULT_TRUNCATE_NOTICE, maybe_truncate
...
class Skill(BaseModel):
    ...
    This model supports both HRAgents-specific fields and AgentSkills standard
    ...
    def load(
        ...
        Supports both HRAgents-specific frontmatter fields and AgentSkills
        ...
        if path.name.lower() == "skill.md":
            ...
            return cls._load_legacy_HRAgent_skill(path, file_content, skill_base_dir)
    ...
    @classmethod
    def _load_legacy_HRAgent_skill(
        cls, path: Path, file_content: str, skill_base_dir: Path | None
    ) -> "Skill":
        """Load a skill from a legacy HRAgents-format file.
...
def load_skills_from_dir(
    ...
    - HRAgents format: skills/*.md files
    - AgentSkills format: skills/skill-name/SKILL.md directories

    Note, legacy repo instructions will not be loaded here.

    Args:
        skill_dir: Path to the skills directory (e.g. .HRAgent/skills)
...
USER_SKILLS_DIRS = [
    ...
    Path.home() / ".HRAgent" / "skills",
    Path.home() / ".HRAgent" / "microagents",  # Legacy support
]
...
def load_user_skills() -> list[Skill]:
    ...
    Searches for skills in ~/.agents/skills/, ~/.HRAgent/skills/, and
    ~/.HRAgent/microagents/ (legacy). Skills from all directories are merged,
    with earlier entries in USER_SKILLS_DIRS taking precedence for duplicate
    names.

    Also loads enabled installed skills from ~/.HRAgent/skills/installed/
    ...
    try:
        from skills.installed import load_installed_skills
...
def load_project_skills(work_dir: str | Path) -> list[Skill]:
    ...
    Searches for skills in {work_dir}/.agents/skills/, {work_dir}/.HRAgent/skills/,
    and {work_dir}/.HRAgent/microagents/ (legacy).
    ...
    Use .agents/skills for new skills. .HRAgent/skills is the legacy HRAgents
    location, and .HRAgent/microagents is deprecated.

    Example: If "my-skill" exists in both .agents/skills/ and .HRAgent/skills/,
    ...
    # search root. This ensures they are loaded even if .HRAgent/skills doesn't
    # exist.
    ...
    # Load project-specific skills from .agents/skills, .HRAgent/skills,
    # and legacy microagents (priority order; first wins for duplicates)
    for root in search_roots:
        project_skills_dirs = [
            root / ".agents" / "skills",
            root / ".HRAgent" / "skills",
            root / ".HRAgent" / "microagents",  # Legacy support
        ]
...
PUBLIC_SKILLS_REPO = "https://github.com/HRAgents/extensions"
...
def load_marketplace_skill_names(
    ...
    Uses the existing Marketplace model from plugins to parse
...
def load_public_skills(
    ...
    """Load skills from the public HRAgents skills repository.

    This function maintains a local git clone of the public skills registry at
    https://github.com/HRAgents/extensions. On first run, it clones the repository
    to ~/.HRAgent/skills-cache/. On subsequent runs within the same process, it
    ...
    Example:
        ...
        >>> from skills import load_public_skills
        >>>
        >>> # Load public skills
...
def load_available_skills(
    ...
    *,
    ...
    Args:
        ...
        include_public: Load public skills from the HRAgents extensions repo.
