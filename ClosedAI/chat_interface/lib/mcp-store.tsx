...
function buildConnection(name: string, server: mcpApi.McpServerConfig, existing?: McpConnection): McpConnection {
  ...
  return {
    ...
    isBuiltin: isProtectedMcpServer(name),
    origin: isProtectedMcpServer(name) ? "builtin" : "marketplace",
  }
}
...
export const PROTECTED_MCP_SERVER_IDS = new Set([
  "azure-ai-search",
  "cosmos-db",
  "document-editor",
])

function isProtectedMcpServer(id: string): boolean {
  return PROTECTED_MCP_SERVER_IDS.has(id)
}
...
const MCP_LOAD_RETRIES = 3
const MCP_LOAD_RETRY_MS = 1500

async function withMcpRetry<T>(label: string, fn: () => Promise<T>): Promise<T> {
  let last: unknown
  for (let attempt = 0; attempt < MCP_LOAD_RETRIES; attempt++) {
    try {
      return await fn()
    } catch (error) {
      last = error
      if (attempt < MCP_LOAD_RETRIES - 1) {
        await new Promise((r) => setTimeout(r, MCP_LOAD_RETRY_MS * (attempt + 1)))
      }
    }
  }
...
export function McpProvider({ children }: { children: ReactNode }) {
  ...
  const load = useCallback(async () => {
    try {
      const [settings, secretsRes] = await withMcpRetry("load MCP settings", () =>
        Promise.all([mcpApi.getSettings(), mcpApi.listSecrets().catch(() => ({ secrets: [] }))]),
      )
      ...
      // Soft-fail: MCP panel is non-critical. Never toast on boot — Next overlays
      // "fetch failed" as a console error and panics users when the backend is
      // still starting. Retry quietly; surface only while catalogError stays set.
      console.warn(
        "MCP settings unavailable (backend may still be starting):",
  ...
  const loadCatalog = useCallback(async () => {
    ...
    try {
      const { plugins } = await withMcpRetry("load MCP marketplace", () => mcpApi.marketplaceCatalog())
      setCatalog(
        plugins.map((p) => ({
          id: p.name,
          name: p.name,
      ...
      console.warn(
        "MCP marketplace unavailable (backend may still be starting):",
        error instanceof Error ? error.message : error,
      )
  ...
  const toggleConnection = useCallback(
    async (id: string) => {
      ...
      if (!conn) return
      if (conn.isBuiltin) {
        toast.info(`${conn.name} is a built-in server and cannot be disabled`)
        return
      }
      try {
        if (conn.connected) {
  ...
  const deleteConnection = useCallback(
    async (id: string) => {
      const conn = connections.find((c) => c.id === id)
      if (conn?.isBuiltin) {
        toast.error(`${conn.name} is a built-in server and cannot be uninstalled`)
        return
      }
      try {
        await patchConfig({ [id]: null })
  ...
  const disconnectAll = useCallback(async () => {
    const toRemove = connections.filter((c) => c.connected && !c.isBuiltin).map((c) => c.id)
    if (toRemove.length === 0) {
      toast.info("Core MCP servers can’t be disconnected from here")
      return
    }
    try {
